...
class TestDtpStateDerivation:
    ...
    def test_active_false_returns_cancelado(self):
        ...
        assert (
            _derive_dtp_state(
                {"active": "false", "approval": "not requested", "state": "4"}
            )
            == "Cancelado"
        )
    ...
    def test_active_true_approved_returns_aprovada(self):
        ...
        assert (
            _derive_dtp_state({"active": "true", "approval": "approved", "state": "-1"})
            == "Aprovada"
        )
    ...
    def test_active_true_requested_returns_aguardando(self):
        ...
        assert (
            _derive_dtp_state(
                {"active": "true", "approval": "requested", "state": "-3"}
            )
    ...
    def test_active_true_rejected_returns_rejeitada(self):
        ...
        assert (
            _derive_dtp_state({"active": "true", "approval": "rejected", "state": "-1"})
            == "Rejeitada"
        )
    ...
    def test_state_closed_returns_implementada(self):
        ...
        assert (
            _derive_dtp_state({"active": "true", "approval": "", "state": "0"})
            == "Implementada"
        )
    ...
    def test_fallback_returns_state_value(self):
        ...
        assert (
            _derive_dtp_state({"active": "true", "approval": "", "state": "Revisão"})
            == "Revisão"
        )

    @patch("servicenow_mcp.tools.itsm_tools.make_sn_request")
    @patch("servicenow_mcp.tools.itsm_tools.get_config")
    def test_change_search_includes_dtp_state(
        self, mock_config, mock_request, sn_config
    ):
        mock_config.return_value = sn_config
        mock_request.return_value = _make_response(
            {
    ...
    @patch("servicenow_mcp.tools.itsm_tools.make_sn_request")
    @patch("servicenow_mcp.tools.itsm_tools.get_config")
    def test_change_search_fields_include_active_approval(
        self, mock_config, mock_request, sn_config
    ):
        mock_config.return_value = sn_config
        mock_request.return_value = _make_response({"result": []})
